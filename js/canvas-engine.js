...
const CanvasEngine = {
    ...
    enforceZIndex() {
        ...
        // Z-Index hierarchy: 0=Grid/Walls, 1=Rugs, 2=Desks/Cabinets, 3=Windows
        objects.sort((a, b) => {
            const getZ = (obj) => {
        ...
        objects.forEach((obj, idx) => {
            this.fabricCanvas.moveTo(obj, idx);
        });
    },
        ...
        this.fabricCanvas.getObjects().forEach(obj => {
            if (obj.isFurniture) {
                furnitureManifest.push({
                    ...
                    isPositionLocked: obj.isPositionLocked || false
                });
        ...
        try {
            ...
            furnitureManifest.forEach(f => {
                ...
                if (f.furnitureType === 'row' || f.furnitureType === 'single' || f.furnitureType === 'pod') {
                    ...
                    if (deskGroup && this.isGlobalDeskLocked) {
                        deskGroup.set({ lockMovementX: true, lockMovementY: true, lockRotation: true, lockScalingX: true, lockScalingY: true, hasControls: false, hoverCursor: 'default' });
                    }
    ...
    buildAssetObject(assetType, width, height, fill, stroke, labelText, textFill, shape = 'rect') {
        ...
        if (shape === 'circle') {
            baseShape = new fabric.Circle({
                ...
                fill: fill, stroke: stroke, strokeWidth: 2, strokeUniform: true
            });
        } else if (shape === 'half_circle') {
            const pathStr = `M 0 ${height} A ${width/2} ${height} 0 0 1 ${width} ${height} Z`;
            baseShape = new fabric.Path(pathStr, {
                left: 0, top: 0, fill: fill, stroke: stroke, strokeWidth: 2, strokeUniform: true
            });
        } else {
            baseShape = new fabric.Rect({ 
                left: 0, top: 0, width: width, height: height, 
                fill: fill, stroke: stroke, strokeWidth: 2, rx: (assetType.includes('rug') ? 12 : 2), ry: (assetType.includes('rug') ? 12 : 2), strokeUniform: true 
            });
        }
    ...
    spawnRow(count, dW, dL) {
        ...
        if (this.isGlobalDeskLocked) {
            group.set({ lockMovementX: true, lockMovementY: true, lockRotation: true, lockScalingX: true, lockScalingY: true, hasControls: false, hoverCursor: 'default' });
        }
    ...
    attachControlListeners() {
        ...
        // 1. INVERSE SCALING DURING DRAG
        this.fabricCanvas.on('object:scaling', (options) => {
            const obj = options.target;
            if (obj && obj.isFurniture) {
                obj.getObjects().forEach(child => {
                    // Only inverse scale the text to prevent font stretching
                    if (child.type === 'textbox' || child.type === 'text') {
                        child.set({
                            scaleX: 1 / obj.scaleX,
                            scaleY: 1 / obj.scaleY
            ...
            if (obj && obj.isFurniture && !obj.seats && obj.blueprint) {
                if (obj.scaleX !== 1 || obj.scaleY !== 1) {
                    ...
                    newGroup.set({
                        ...
                        angle: obj.angle,
                        furnitureId: obj.furnitureId,
                        isPositionLocked: obj.isPositionLocked
                    });
        ...
        // 3. ENHANCED DOUBLE-CLICK LISTENER
        this.fabricCanvas.upperCanvasEl.addEventListener('dblclick', (e) => {
            ...
            if (activeObj && activeObj.isFurniture) {
                ...
                // Behavior A: It is a desk -> Lock/Unlock the student seat
                if (activeObj.seats) {
                ...
                // Behavior B: It is an environmental asset -> Rename and toggle Position Lock
                else {
                    ...
                    activeObj.set({
                        lockMovementX: activeObj.isPositionLocked,
                        lockMovementY: activeObj.isPositionLocked,
                        lockRotation: activeObj.isPositionLocked,
                        lockScalingX: activeObj.isPositionLocked,
                        lockScalingY: activeObj.isPositionLocked,
                        hasControls: !activeObj.isPositionLocked
                    });
